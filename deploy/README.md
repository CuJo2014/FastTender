...
**Compose ругается на "variable not set" и хеш в BASIC_AUTH_HASH разломан:**
- Каждый `$` в bcrypt-хеше нужно эскейпить как `$$` в `.env.prod`. Compose
  интерпретирует `${...}` в значениях, и `$2a$14$abcd` для него выглядит
  как «подставь переменную `2a` без значения, потом `14`, …». `bootstrap.sh
  basic-auth` уже выводит правильно эскейпленную версию; при ручной правке
  не забыть.
