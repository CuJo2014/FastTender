...
## Авто-очистка деактивированных (с 2026-06-02)

Ежедневный cron в **04:20 UTC** запускает `deploy/cleanup-deactivated.sh`
который удаляет «осиротевшие» deactivated позиции — те на которые
**нет ссылок** из `match_candidate` или `verification`. Позиции с
живыми ссылками остаются для сохранения истории матчей и решений
менеджеров.

Лог: `logs/cleanup.log`. Проверить:
```bash
tail -10 /home/master/fasttender/logs/cleanup.log
```

Ручной запуск:
```bash
/home/master/fasttender/deploy/cleanup-deactivated.sh
```

## Ручная чистка деактивированных позиций
