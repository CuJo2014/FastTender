...
#
# Клиенты нередко сваливают в ячейку наименования целое ТЗ: сперва имя товара,
# а следом — комплектацию, опции, сертификацию (НАКС/ГОСТ/ТУ/РД), упаковку,
# техпаспортные параметры. Текст ПОСЛЕ такого «канцелярского» маркера почти не
# несёт различающего сигнала для лексического поиска, зато раздувает запрос:
# триграмм-similarity по простыне ≈0, а ts_rank цепляет чужие позиции по
# случайным словам (генератор, кабель, вилка, гост…). Поэтому для ПОИСКА
# обрезаем наименование по первому такому маркеру. Сырое имя (name_raw) и
# извлечение кодов/бренда продолжают работать по полному тексту.
_BOILERPLATE_MARKERS = re.compile(
    ...
    r"|соответстви\w*"
    r"|требовани\w*"
    r"|завод\w*"  # «завод ESAB в Санкт-Петербурге …» — происхождение, не товар
    r"|гост|ту\s*\d|рд\s*\d"
    r"|упаковк\w*"
    r"|напряжение\s+сети"
    r"|температур\w*"
    r"|габарит\w*"
    r"|исполнени\w*"
    r"|категори\w*\s+помещени\w*"
    r")\b",
    re.IGNORECASE,
)

# «Голову» короче этого (символов / токенов) считаем «срезали слишком много» —
# маркер-слово оказалось самим товаром («Датчик температуры», «Упаковочная
# лента»): тогда денойз отменяем и ищем по полному тексту.
_MIN_DENOISED_CHARS = 10
_MIN_DENOISED_TOKENS = 2


def denoise_name(value: Any) -> str | None:
    """Обрезает «канцелярский хвост» клиентского наименования для поиска.

    Возвращает «голову» до первого boilerplate-маркера (комплектация, опции,
    ГОСТ/ТУ/РД, упаковка, техпараметры…). Если маркера нет — текст без
    изменений. Если голова вышла подозрительно короткой (маркер сам и есть
    товар) — откатываемся к полному тексту. См. вариант A в docs/BACKLOG.md.
    """
    s = clean_string(value)
    if s is None:
        return None
    m = _BOILERPLATE_MARKERS.search(s)
    if m is None:
        return s
    head = s[: m.start()].strip(" .,:;–—-+/")
    if len(head) < _MIN_DENOISED_CHARS or len(head.split()) < _MIN_DENOISED_TOKENS:
        return s
    return head
